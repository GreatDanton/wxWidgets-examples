#include "mainframe.h"
#include "scrolledwindow.h"
#include <memory>

MainFrame::MainFrame(const wxString &title)
    : wxFrame(nullptr, -1, title, wxDefaultPosition, wxSize(500, 500))
{
    wxMenuBar *menubar = new wxMenuBar();

    // File menu
    wxMenu *file = new wxMenu();
    wxMenuItem *open = new wxMenuItem(file, -1, wxT("&Open"));
    wxMenuItem *close = new wxMenuItem(file, -1, wxT("&Quit"));

    file->Append(open);
    file->AppendSeparator();
    file->Append(close);

    // file/open command
    Connect(open->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(MainFrame::fileOpen));
    // file/quit command
    Connect(close->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(MainFrame::quit));

    menubar->Append(file, wxT("&File"));
    SetMenuBar(menubar);

    // MAIN FRAME: display main application window
    m_mainWindow = new ScrolledWindow(this);

    // center app window
    Centre();
}

void MainFrame::fileOpen(wxCommandEvent &WXUNUSED(event))
{
    wxFileDialog dialog(m_mainWindow, wxT("Load image"));
    int userClick = dialog.ShowModal();

    if (userClick == wxID_OK)
    {
        wxString filePath = dialog.GetPath();
        // load image
        std::shared_ptr<wxBitmap> opened_image = std::make_shared<wxBitmap>();
        opened_image->LoadFile(filePath, wxBITMAP_TYPE_ANY);
        // display image in the placeholder
        m_mainWindow->setImage(opened_image);
    }
    else
    {
        std::cout << "File dialog canceled" << std::endl;
    }
}

void MainFrame::quit(wxCommandEvent &WXUNUSED(event))
{
    Close(true);
}
